...
if (!empty($arResult['ITEMS']))
{
	...
	foreach ($arResult['ITEMS'] as $key => $arItem)
	{
		...
		if ($arItem['CATALOG'] && isset($arItem['OFFERS']) && !empty($arItem['OFFERS']))
		{
          ...
          // TODO: SUPER DIRTY HACK IN BITRIX STYLE! 
          if ($arOffer['TREE']['PROP_42']!=$city) {
              //continue;
          } else {
              $intSelected = $keyOffer;
              $arItem['MIN_PRICE'] = (isset($arOffer['RATIO_PRICE']) ? $arOffer['RATIO_PRICE'] : $arOffer['MIN_PRICE']);
				...
				if (-1 == $intSelected)
					$intSelected = 0;
        $arItem['MIN_PRICE2'] = $arItem['MIN_PRICE']["VALUE"];
...
{ 
    ...
    if($sort[0] == "PRICE") {
        $sort_dir = strtolower($sort[1])=="desc" ? -1.0 : 1.0;
        usort($arNewItemsList, function ($item1, $item2) use ($sort_dir) {
            return ($item1['MIN_PRICE2'] - $item2['MIN_PRICE2'])*$sort_dir;
        });        
...
$res = CIBlockSection::GetList(array(),array(
	...
	"ID" => $arResult['ID'],
),false,array("ID", "UF_SHOW_TITLE", "UF_IMAGE_NSK", "UF_IMAGE_KEM", "UF_IMAGE_NKZ"));

if ($row = $res->GetNext()) {
	
	//$k = (int)$row['UF_SHOW_TITLE'];

	$arResult['UF_SHOW_TITLE'] = (int)$row['UF_SHOW_TITLE'];
	$arResult['UF_IMAGE_NSK'] = $row['UF_IMAGE_NSK'];
	$arResult['UF_IMAGE_NKZ'] = $row['UF_IMAGE_NKZ'];
	$arResult['UF_IMAGE_KEM'] = $row['UF_IMAGE_KEM'];
}
