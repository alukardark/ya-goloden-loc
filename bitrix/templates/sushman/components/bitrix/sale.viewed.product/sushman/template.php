                ...
                <div class="swiper-slide">
                    <div class="bx_catalog_item_container">
                        ...
                        <a href="<? echo $arItem['DETAIL_PAGE_URL']; ?>"
                           data-popup_href="/menu/detail.php?SECTION_ID=&ELEMENT_ID=<?= $arItem["PRODUCT_ID"] ?>"
                        ...
                        <? if (!in_array($arItem["SECTION_ID"], $hidden_sections)) { ?>
                            ...
                            <span class="price"><?= str_replace("руб.", "<span class='ico-rub'></span>",
                                    ($found ? $found["PRICES"]["BASE"]["PRINT_DISCOUNT_VALUE"] : $arItem["PRICE_FORMATED"])) ?></span>
                            </div>
                            <?

                            if (!in_array($arItem["SECTION_ID"], $ignored_ids)) { ?>
                                <a onclick="ga('send', 'event', 'Basket <?= strtoupper($cityCode == 'nkz' ? 'nk' : $cityCode) ?>', 'Add product <?= strtoupper($cityCode == 'nkz' ? 'nk' : $cityCode) ?>')" href="<?= $found ? $found["BUY_URL"] : $arItem["BUY_URL"] ?>"
                                   data-add-url="<?= $found ? $found["ADD_URL"] : $arItem["ADD_URL"] ?>" rel="nofollow"
                                   class="buy">Купить</a>
                            <? } else {
                                ?>
                                <a href="#" data-id="<?= $arItem["PRODUCT_ID"] ?>" rel="nofollow"
                                   class="buy btn-addto-constructor <?= (in_array($arItem["SECTION_ID"],
                                       $wok_section_ids) ? "wok-cons" : "") . (in_array($arItem["SECTION_ID"],
