...
$arJSParams = array(
			...
			'PRODUCT' => array(
				...
				'ADD_URL' => str_replace('/search/','/menu/',$arItem['~ADD_URL']),
				'SUBSCRIBE_URL' => str_replace('/search/','/menu/',$arItem['~SUBSCRIBE_URL'])
			),
			'BASKET' => array(
...
<? } ?>                
                                    ...
                                    ?>
                                    <a href="<? echo $arItem['DETAIL_PAGE_URL']; ?>" data-popup_href="/menu/detail.php?SECTION_ID=<?=$arItem["SECTION_ID"]?>&ELEMENT_ID=<?=$arItem["ID"]?>" id="<? echo $arItemIDs['PICT']; ?>" class="pic" title="<? echo $strTitle; ?>"><img src="<?=$preview['src']?>"/></a>
                                    <a class="title valigned d" href="<? echo $arItem['DETAIL_PAGE_URL']; ?>" title="<? echo $arItem['NAME']; ?>"><span><? echo $arItem['NAME']; ?></span></a>
...
<?
	...
	if (!empty($arItem['MIN_PRICE']))
	{
		...
		{
			echo str_replace("руб.","<span class='ico-rub'></span>",GetMessage(
...
if (in_array($arItem["IBLOCK_SECTION_ID"],$ignored_ids)) {
    $canBuy = false;
    $arResult['CAN_BUY'] = "N";
	$buyBtnClass = 'bx_big bx_bt_button bx_cart btn-addto-constructor '.(in_array($arItem["IBLOCK_SECTION_ID"],$wok_section_ids) ? "wok-cons" : "").(in_array($arItem["IBLOCK_SECTION_ID"],$pizza_section_ids) ? "pizza-cons" : "");
...
<? } ?>
	...
	{
    if (in_array($arItem["IBLOCK_SECTION_ID"],$ignored_ids)) { ?> 
            <a href="#" data-id="<?=$arItem["ID"]?>" rel="nofollow" class="buy btn-addto-constructor <?=(in_array($arItem["IBLOCK_SECTION_ID"],$wok_section_ids) ? "wok-cons" : "").(in_array($arItem["IBLOCK_SECTION_ID"],$pizza_section_ids) ? "pizza-cons" : "")?>">Добавить</a>
    <? } else {
		if ($canBuy)
		{
			?>
			<a id="<? echo $arItemIDs['BUY_LINK']; ?>" class="buy bx_bt_button bx_medium <?=$buyBtnClass?>" href="javascript:void(0)" rel="nofollow"><?
			echo $buyBtnMessage;
			?></a>
			<?
		}
