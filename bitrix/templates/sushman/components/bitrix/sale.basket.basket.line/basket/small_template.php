...
<div class="bx_small_cart">
	...
	<?if ($arResult['NUM_PRODUCTS'] > 0 && $arParams['SHOW_NUM_PRODUCTS'] == 'N' && $arParams['SHOW_TOTAL_PRICE'] == 'N'):?>
		<a href="<?=$arParams['PATH_TO_BASKET']?>"<? if (!$arResult['TOTAL_PRICE']) {?> style="background-image:url(/bitrix/templates/sushman/img/icon-cart-disabled.png);" onclick="javascript:return false;"<? }?>><?=GetMessage('TSB1_CART')?></a>
	<?else: echo GetMessage('TSB1_CART'); endif?>

	<?if($arParams['SHOW_NUM_PRODUCTS'] == 'Y'):?>
		<?if ($arResult['NUM_PRODUCTS'] > 0):?>
			<a href="<?=$arParams['PATH_TO_BASKET']?>"<? if (!$arResult['TOTAL_PRICE']) {?> style="background-image:url(/bitrix/templates/sushman/img/icon-cart-disabled.png);" onclick="javascript:return false;"<? }?>><?=$arResult['NUM_PRODUCTS'].' '.$arResult['PRODUCT(S)']?></a>
		<?else:?>
