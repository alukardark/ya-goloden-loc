BX.ready(function () {
    var sku_props = BX.findChildren(BX('basket_items'), {
        tagName: 'li',
        className: 'sku_prop'
    }, true);
...
function skuPropClickHandler(e) {
    ...
    if (!!target && target.hasAttribute('data-value-id')) {
        ...
        var all_sku_props = BX.findChildren(BX(basketItemId), {
            tagName: 'ul',
...
function updateBasketTable(basketItemId, res) {
    ...
    if (basketItemId !== null) {
        ...
        for (i = 0; i < arColumns.length; i++) {
            ...
            if (arColumns[i] == 'NAME') {
                ...
                if (typeof(arItem[arColumns[i]]) != 'undefined') {
                    customColumnVal += arItem[arColumns[i]].replace('руб.', "<span class='ico-rub'></span>");
                }
        ...
        var sku_props = BX.findChildren(BX(newBasketItemId), {
            tagName: 'li',
            className: 'sku_prop'
        }, true);
        if (!!sku_props && sku_props.length > 0) {
    ...
    for (var id in res.BASKET_DATA.GRID.ROWS) {
        if (res.BASKET_DATA.GRID.ROWS.hasOwnProperty(id)) {
            ...
            if (BX('sum_' + id))
                BX('sum_' + id).innerHTML = item.SUM.replace('руб.', "<span class='ico-rub'></span>");

            // if the quantity was set by user to 0 or was too much, we need to show corrected quantity value from ajax response
            if (BX('QUANTITY_' + id)) {
                BX('QUANTITY_INPUT_' + id).value = item.QUANTITY;
                BX('QUANTITY_INPUT_' + id).defaultValue = item.QUANTITY;
                if (BX('QUANTITY_SELECT_' + id)) {
    ...
    var totalCount = 0;
    for (var i = 0; i < res['BASKET_DATA']['ITEMS']['AnDelCanBuy'].length; i++) {
        totalCount += 1 * res['BASKET_DATA']['ITEMS']['AnDelCanBuy'][i]["QUANTITY"];
    }
    if (BX('allCount'))
        BX('allCount').innerHTML = totalCount;
    ...
    if (BX('allSum_FORMATED'))
        BX('allSum_FORMATED').innerHTML = res['BASKET_DATA']['allSum_FORMATED'].replace(/\s/g, '&nbsp;').replace('руб.', "<span class='ico-rub'></span>");
}
...
function recalcBasketAjax() {
    ...
    BX.ajax({
        ...
        dataType: 'json',
        onsuccess: function (result) {
            $("#basket_items_list .bg").hide();
            BX.closeWait();
            sbbl.refreshCart();
            updateBasketTable(null, result);
            var prices = {};
            $("#basket_items tr[data-cid]").each(function () {
                var cid = $(this).attr("data-cid");
                if (!prices[cid])
                    prices[cid] = 0;
                prices[cid] += parseInt($(this).find(".sum").text());
            });
            for (var cid in prices) {
                $("#basket_items tr[id=" + cid + "] .sum").html(prices[cid] + " <span class='ico-rub'></span>");
            }
            if (window['submitForm']) {
...
// check if quantity is valid
// and update values of both controls (text input field for PC and mobile quantity select) simultaneously
function updateComplexQuantity(controlId, basketId, ratio, bUseFloatQuantity) {
    var oldVal = BX(controlId).defaultValue,
        newVal = parseFloat(BX(controlId).value) || 0,
        bIsCorrectQuantityForRatio = false;
    ...
    newVal = (bUseFloatQuantity === false && bIsQuantityFloat === false) ? parseInt(newVal) : parseFloat(newVal).toFixed(2);
    $("#basket_items tr[data-cid=" + basketId + "]").each(function () {
        var pid = $(this).attr("id");
        BX("QUANTITY_INPUT_" + pid).value = newVal;
        BX("QUANTITY_INPUT_" + pid).defaultValue = newVal;
        BX("QUANTITY_" + pid).value = newVal;
...
// used when quantity is changed by clicking on arrows
function setComplexQuantity(basketId, ratio, sign, bUseFloatQuantity) {
    var curVal = parseFloat(BX("QUANTITY_INPUT_" + basketId).value),
        newVal;
    ...
    updateComplexQuantity('QUANTITY_INPUT_' + basketId, basketId, ratio, bUseFloatQuantity);
