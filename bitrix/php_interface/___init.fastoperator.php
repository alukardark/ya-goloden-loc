...
global $wok_section_ids,$pizza_section_ids,$ignored_ids;
$wok_section_ids= array(36,37,47,48);
$pizza_section_ids = array(33,49);
...
function is_order_time() {
    global $city;
    $t = getdate();
    if ($t['wday']>=0 && $t['wday']<=4) {
        return ($t['hours']>=10 && ($t['hours']<23 || ($t['hours']==23 && $t['minutes']<=30))) || ($t['wday']==0 && ($t['hours']==0 && $t['minutes']<=30));
    } else {
        return $t['hours']>=10 || ($t['wday']==6 && $t['hours']==0 && $t['minutes']<=30);
    }
    return false;
}
...
class MyClass
{
    ...
    CModule::IncludeModule('highloadblock');    
                    ...
                    $entity = Bitrix\Highloadblock\HighloadBlockTable::compileEntity($hlblock);
                    $main_query = new Bitrix\Main\Entity\Query($entity);
                    $main_query->setSelect(array('ID','UF_XML_ID','UF_NAME','UF_ACTIVE','UF_DELIVERY_PIC','UF_DELIVERY'));
                    $result = $main_query->exec();
                    $result = new CDBResult($result);
                    $cities = array();
                    while ($row = $result->Fetch())
                    {
                        $cities[$row["ID"]] = array("CODE" => $row["UF_XML_ID"],"NAME" => $row["UF_NAME"],'ACTIVE' => $row['UF_ACTIVE'],"PHONE" => $row['UF_PHONE'],"DELIVERY_PIC" => $row['UF_DELIVERY_PIC'],"DELIVERY" => $row['UF_DELIVERY']);
                    }
                    foreach($cities as $c) {
...
function getNonDiscountSections() {
    $res = CIBlockSection::GetList(Array("SORT"=>"ASC"), Array("IBLOCK_ID" => 2, "ACTIVE" => "Y","!UF_NOT_DISCOUNT" => ""), false, Array("ID","UF_NOT_DISCOUNT"));
    $sections = array();
...
$order_discount_name = "";  
  ...
  //ws_add_order($data,$arBasketList);
    ...
    if ($admin_phone) {
      ...
      $params = array(
          "onlydelivery" => "0",
          "text" => "Спасибо за заказ! Ожидайте звонка. я-голоден.рф"
      );
...
function SetGlobalVars() {
    ...
    while ($row = $result->Fetch())
    {
      if ($row['UF_ACTIVE']==17)
        ...
        $cities[$row["ID"]] = array("CODE" => $city_code,"NAME" => $row["UF_NAME"],'ACTIVE' => $row['UF_ACTIVE'],"PHONE" => $row['UF_PHONE'],"DELIVERY_PIC" => $row['UF_DELIVERY_PIC'],"DELIVERY" => $row['UF_DELIVERY'],'ORDER_ENABLED' => $row['UF_ORDER_ENABLED'],'SMS_PHONE' => $row['UF_SMS_PHONE']);
    ...
    if ($city!=1 && $city!=3) {
        if ($cookie_city==1 || $cookie_city==3) {
            $city=$cookie_city;
        } else {
          getCity();
          if ((int)$city!=1 && (int)$city!=3) {
            $city=1;
          }
...
function recalcBasket() {
    ...
    if (!empty($arID))
         ...
         array(
            ...
            false,
            false,
            array("ID", "CALLBACK_FUNC", "MODULE", "PRODUCT_ID", "QUANTITY", "DELAY", "CAN_BUY", "PRICE", "WEIGHT", "PRODUCT_PROVIDER_CLASS", "NAME","SUBSCRIBE")
        );
        while ($arItem = $dbBasketItems->Fetch())
        {
...
function ws_add_order($data,$products_data) {
    ...
    foreach ($products_data as $arItem)
    {
        ...
        if($ar_res = $res->GetNext()) {
            $db_props = CIBlockElement::GetProperty($ar_res["IBLOCK_ID"], $ar_res["ID"], array(), Array("CODE" => "FOID"));
