	...
	if ('D' == $arCatalog['CATALOG_TYPE'] || 'O' == $arCatalog['CATALOG_TYPE'])
	{
		...
		while ($obElement = $res->GetNextElement())
		{
			...
			$rsProducts = CCatalogProduct::GetList(
				...
				false,
				false,
				array('ID', 'WEIGHT', 'QUANTITY', 'QUANTITY_TRACE', 'CAN_BUY_ZERO')
			);
...
/*					$dbVAT = CCatalogProduct::GetVATInfo($arAcc['ID']);
			...
			$strTmpOff.= "<url>http://".$ar_iblock['SERVER_NAME'].htmlspecialcharsbx($arAcc["~DETAIL_PAGE_URL"]).(strstr($arAcc['DETAIL_PAGE_URL'], '?') === false ? '?' : '&amp;')."</url>\n";

			$strTmpOff.= "<price>".$minPrice."</price>\n";
			$strTmpOff.= "<weight>".$arAcc['WEIGHT']."</weight>\n";
...
/*							$dbVAT = CCatalogProduct::GetVATInfo($arOfferItem['ID']);
				...
				if (!empty($arCurrentOffer) && !empty($arCurrentPrice))
				{
					...
					if ($arProduct = $rsProducts->Fetch())
					{
						...
						$arOfferItem['WEIGHT'] = $arProduct['WEIGHT'];
					}
					...
					$strOfferYandex .= "<url>http://".$ar_iblock['SERVER_NAME'].htmlspecialcharsbx($arOfferItem["~DETAIL_PAGE_URL"]).(strstr($arOfferItem['DETAIL_PAGE_URL'], '?') === false ? '?' : '&amp;')."</url>\n";

					$strOfferYandex .= "<price>".$minPrice."</price>\n";
					$strOfferYandex .= "<weight>".$arOfferItem['WEIGHT']."</weight>\n";
...
/*							$dbVAT = CCatalogProduct::GetVATInfo($arOfferItem['ID']);
			...
			if ('X' == $arCatalog['CATALOG_TYPE'] && !$boolItemOffers)
			{
				...
				if ($arProduct = $rsProducts->Fetch())
				{
					...
					$arItem['WEIGHT'] = $arProduct['WEIGHT'];
				}
...
/*						$dbVAT = CCatalogProduct::GetVATInfo($arItem['ID']);
				...
				$strOfferYandex.= "<url>http://".$ar_iblock['SERVER_NAME'].htmlspecialcharsbx($arItem["~DETAIL_PAGE_URL"]).(strstr($arItem['DETAIL_PAGE_URL'], '?') === false ? '?' : '&amp;')."</url>\n";

				$strOfferYandex.= "<price>".$minPrice."</price>\n";
				$strOfferYandex.= "<weight>".$arItem['WEIGHT']."</weight>\n";
				$strOfferYandex.= "<currencyId>".$minPriceCurrency."</currencyId>\n";
