<?php	
		...
		if ($parse_it->OrderStatus[0]=="CANCELED"){
		 header("Location: ".$url.$amp."status=".$parse_it->OrderStatus[0]."&desc=".$desc);
		 }
		else {
		  ...
		  $parse_it = simplexml_load_string($xml_string);
		   $desc = (string)$parse_it->ResponseDescription;
		  if ($parse_it->OrderStatus[0]=="DECLINED" OR $parse_it->OrderStatus[0]=="APPROVED"){
		  	header("Location: ".$url.$amp."status=".$parse_it->OrderStatus[0]."&desc=".$desc);
		  } 
