...
$APPLICATION->SetTitle("Поиск по ингредиентам");
...
?>
    ...
    <div class="main-col">
        <div id="sect-pic" class="no-image">
            <h2 class="sect sect-title">Поиск по ингредиентам</h2>
        </div>
        <div id="search-by">
            <?
            $_GET['sort_id'] = "UF_SORT";
            $_GET['sort_type'] = "ASC";
            $APPLICATION->IncludeComponent("bitrix:highloadblock.list", "search", Array(
                    "BLOCK_ID" => "3"
                )
            ); ?>
        </div>
        ...
        $arFilter = array();
        foreach ($_POST['ingr'] as $ingr) {
            $ingr_id = (int)$ingr;
            if ($ingr_id > 0 && $ingrs[$ingr_id]) {
                $arFilter[] = array(
                    "ID" => CIBlockElement::SubQuery("ID",
                        array("IBLOCK_ID" => 2, "PROPERTY_TYPE_REF" => $ingrs[$ingr_id]))
                );
            }
        }
        if (count($arFilter) == 0) {
