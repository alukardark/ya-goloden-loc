...
if(CModule::IncludeModule("iblock")) {
    ...
    $res = CIBlockSection::GetList($arOrder, $arFilter, array("ELEMENT_SUBSECTIONS" => "Y","CNT_ACTIVE" => "Y"), $arSelect);
    $i=0;
    while($arFields = $res->GetNext()) // наполняем массив меню пунктами меню
    {
        ...
        $aMenuLinksExt[] = Array(
            ...
            "/menu/{$arFields['CODE']}/",
            $add,
            Array("CODE" => $arFields['CODE'], "IMAGE" => $pic["src"],"props" => $vals,"alt" => $vals["SECTION_PICTURE_FILE_ALT"],"title" => $vals["SECTION_PICTURE_FILE_TITLE"]),
            ""
        );
        $i++;
    }   
